...
`Validator` (or typically, a `LocalValidatorFactoryBean`) is configured on the `AnnotatedControllerConfigurer`,
controller arguments annotated with `@Valid` and `@Validated` are validated before method invocation.
